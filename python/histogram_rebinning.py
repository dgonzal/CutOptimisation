...
def binFile(rerror, filename, xtitle, backgrounds): 
    ...
    for key in h_bkg:
        ...
        maxs = h_bkg[key].GetMaximum()#maxs = [h_data[key].GetMaximum(), h_bkg[key].GetMaximum()]
        ...
        legend.AddEntry(h_bkg[key], "Background", "lp")
        ...
        canvas.SaveAs(filename.split('.')[0]+'_'+key+'.pdf')
        ...
        for hkey in keys:
            ...
            if info.channel == key:
                ...
                if info.process.startswith('zp') or info.process.startswith('rsg'):
                    print "Scaling signal sample %s by x1" % histogram.GetName()
                    histogram.Scale(1)
                """    
...
#binFile(0.3, 'boosted_semileptonic_muon_interpolated.root', 'M_{t#bar{t}} [GeV/c^{2}]', ['ttbar','qcd', 'wlight','zlight', 'singletop', 'diboson'])
#binFile(0.3, 'boosted_semileptonic_muon_interpolated.root', 'M_{t#bar{t}} [GeV/c^{2}]', ['ttbar','qcd', 'wlight','zlight'])

#binFile(0.3, 'boosted_semileptonic_lepton_interpolated.root', 'M_{t#bar{t}} [GeV/c^{2}]', ['ttbar', 'wlight', 'wc', 'wb',
# 'zlight', 'singletop', 'diboson'])
